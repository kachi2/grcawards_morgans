                    ...
                    <div class="accordion-wrapper" style="margin-top: 30px;"> 
                        <div class="accordion">
                            ...
                            <div class="accordion-content">
                                <p> {{$award->description != null ? $award->description : ''}}</p>
                                <h6>{{$award->criteria != null ? 'Award Criteria' : ''}} </h6>
                                <p>{{$award->criteria != null ? $award->criteria : ''}}</p>
                                <form action="{{route('landing.index')}}" id="{{$award->hashid}}2">
                                    <div class="buy-ticket-left">
                                        <p>{{$award->has_nominees == true ? 'The following are the nominees for this Award sub-category.' : '' }}</p>
                                        @if($award->sector->category->is_non_voting_category == false)
                                        <p><strong>Please place your vote by {{$award->has_nominees == false ? 'stating the name of your nominee and clicking the "Vote" button' : 'selecting a nominee and clicking the "Vote" button after the list' }}.</strong></p>
                                        @else
                                        <p><strong>This is a non-voting category</strong></p>
                                        @endif
                                        <div class="ticket-contact-item" style="margin-right:0px">
                                            <div class="buy-ticket-form">
                                                @csrf
                                                @if ($award->has_nominees == false)
